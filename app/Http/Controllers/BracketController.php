	...
	public function show($id)
	{
		...
		if(Session::has('products')){
			...
			foreach ($products as $myProduct) {
				$advertisement = Advertisement::findOrFail($myProduct->advertisement_id);
				
				if($myProduct->advertisement_id == $product->advertisement_id){
					$myProduct->number_of_copies++;
					$isProductInArray = true;
				}
				if($advertisement->number_of_copies < $myProduct->number_of_copies){
					...
					$myProduct->number_of_copies = $advertisement->number_of_copies;
				}
			}
		...
		if(count($errors) > 0){
			return redirect('/bracket')->withErrors($errors);
		}
